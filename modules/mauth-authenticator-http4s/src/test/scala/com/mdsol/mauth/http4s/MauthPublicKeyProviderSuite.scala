...
class MauthPublicKeyProviderSuite
  ...
  test("MauthPublicKeyProvider retrieve PublicKey from MAuth Server") {
    ...
    new MauthPublicKeyProvider[IO](
      getMAuthConfiguration, signer = signer, client = Client.fromHttpApp(executeRequest(FakeMAuthServer.EXISTING_CLIENT_APP_UUID.toString, Ok(FakeMAuthServer.mockedMauthTokenResponse())))
    ).getPublicKey(
      FakeMAuthServer.EXISTING_CLIENT_APP_UUID
    ).map(_.nonEmpty).assertEquals(true)
  }
  ...
  test("fail on invalid response from MAuth Server") {
    ...
    new MauthPublicKeyProvider[IO](
      getMAuthConfiguration, signer = mockedSigner, client = Client.fromHttpApp(executeRequest(FakeMAuthServer.NON_EXISTING_CLIENT_APP_UUID.toString, IO(Response[IO](status = Unauthorized))))
    ).getPublicKey(
      FakeMAuthServer.NON_EXISTING_CLIENT_APP_UUID
    ).map(_.nonEmpty).assertEquals(false)
  }
