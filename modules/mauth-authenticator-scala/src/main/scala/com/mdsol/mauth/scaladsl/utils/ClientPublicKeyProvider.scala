...
trait ClientPublicKeyProvider[F[_]] {
   ...
   * @return An effectful value `F[Option[java.security.PublicKey]]` representing the public key registered in MAuth for the application with the given `appUUID`, if one exists.
   */
