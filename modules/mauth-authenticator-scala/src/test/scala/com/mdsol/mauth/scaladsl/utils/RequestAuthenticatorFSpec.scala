...
class RequestAuthenticatorFSpec extends AnyFlatSpec with RequestAuthenticatorBaseSpec with Matchers with ScalaFutures with MockFactory {

  import cats.effect.unsafe.implicits.global

  private implicit val requestValidationTimeout: Duration = 10.seconds

  behavior of "RequestAuthenticatorF Scala"
  ...
  it should "authenticate a valid request" in clientContext { client =>
    ...
    val authenticator = new RequestAuthenticatorF(client, mockEpochTimeProvider)

    whenReady(authenticator.authenticate(getSimpleRequest).unsafeToFuture())(validationResult => validationResult shouldBe true)
  }
  ...
  it should "authenticate a request with unicode chars in body" in clientContext { client =>
    ...
    whenReady(authenticator.authenticate(getRequestWithUnicodeCharactersInBody).unsafeToFuture())(validationResult => validationResult shouldBe true)
  }
  ...
  it should "authenticate a request without any body" in clientContext { client =>
    ...
    whenReady(authenticator.authenticate(getRequestWithoutMessageBody).unsafeToFuture())(validationResult => validationResult shouldBe true)
  }
  ...
  it should "not authenticate an invalid request" in clientContext { client =>
    ...
    whenReady(authenticator.authenticate(invalidRequest).unsafeToFuture())(validationResult => validationResult shouldBe false)
  }
  ...
  it should "not authenticate a request after timeout period passed" in {
    ...
    val authenticator = new RequestAuthenticatorF(mock[ClientPublicKeyProvider[IO]], mockEpochTimeProvider)
  ...
  it should "authenticate a valid request with V2 headers" in clientContext { client =>
    ...
    whenReady(authenticator.authenticate(getSimpleRequestV2).unsafeToFuture())(validationResult => validationResult shouldBe true)
  }

  it should "authenticate a valid request with V2 headers only if V2 only enabled" in clientContext { client =>
    //noinspection ConvertibleToMethodValue
    (mockEpochTimeProvider.inSeconds _: () => Long).expects().returns(CLIENT_X_MWS_TIME_HEADER_VALUE.toLong + 3)
    val authenticator = new RequestAuthenticatorF(client, mockEpochTimeProvider, true)

    val result = authenticator.authenticate(getSimpleRequestV2).unsafeToFuture().futureValue
  ...
  it should "authenticate a valid request with the both V1 and V2 headers provided" in clientContext { client =>
    ...
    whenReady(authenticator.authenticate(getRequestWithAllHeaders).unsafeToFuture())(validationResult => validationResult shouldBe true)
  }

  it should "reject a request with V1 headers when V2 only is enabled" in {
    //noinspection ConvertibleToMethodValue
    (mockEpochTimeProvider.inSeconds _: () => Long).expects().returns(CLIENT_X_MWS_TIME_HEADER_VALUE.toLong + 3)
    val authenticator = new RequestAuthenticatorF(mock[ClientPublicKeyProvider[IO]], mockEpochTimeProvider, true)
  ...
  it should "authenticate a valid request with binary payload" in {
    val client: ClientPublicKeyProvider[IO] = mock[ClientPublicKeyProvider[IO]]
    (client.getPublicKey _).expects(UUID.fromString(CLIENT_REQUEST_BINARY_APP_UUID)).returns(IO(Some(MAuthKeysHelper.getPublicKeyFromString(PUBLIC_KEY2))))
    (mockEpochTimeProvider.inSeconds _: () => Long).expects().returns(CLIENT_REQUEST_BINARY_TIME_HEADER_VALUE.toLong + 3)
    val authenticator = new RequestAuthenticatorF(client, mockEpochTimeProvider)

    whenReady(authenticator.authenticate(getRequestWithBinaryBodyV1).unsafeToFuture())(validationResult => validationResult shouldBe true)
  }
  ...
  it should "authenticate a valid request with binary payload for V2" in {
    ...
    whenReady(authenticator.authenticate(getRequestWithBinaryBodyV2).unsafeToFuture())(validationResult => validationResult shouldBe true)
  }
  ...
  it should "validate the request with the validated V1 headers and wrong V2 signature" in clientContext { client =>
    ...
    whenReady(authenticator.authenticate(getRequestWithWrongV2Signature).unsafeToFuture())(validationResult => validationResult shouldBe true)
  }
  ...
  it should "fail validating request with validated V1 headers and wrong V2 signature if V2 only is enabled" in clientContext { client =>
    ...
    whenReady(authenticator.authenticate(getRequestWithWrongV2Signature).unsafeToFuture())(validationResult => validationResult shouldBe false)
  }
  ...
  "When payload is inputstream" should "authenticate a valid request for V1" in {
    ...
    whenReady(authenticator.authenticate(getRequestWithStreamBodyV1).unsafeToFuture())(validationResult => validationResult shouldBe true)
  }
  ...
  it should "authenticate a valid request for V2" in {
    ...
    whenReady(authenticator.authenticate(getRequestWithStreamBodyV2).unsafeToFuture())(validationResult => validationResult shouldBe true)
  }
  ...
  it should "fail validating the request with the validated V1 headers and wrong V2 signature" in clientContext { client =>
    ...
    whenReady(authenticator.authenticate(getRequestWithStreamBodyAndWrongV2Signature).unsafeToFuture())(validationResult => validationResult shouldBe false)
  }

  private def clientContext(test: ClientPublicKeyProvider[IO] => Any): Unit = {
    val client: ClientPublicKeyProvider[IO] = mock[ClientPublicKeyProvider[IO]]
    (client.getPublicKey _).expects(EXISTING_CLIENT_APP_UUID).returns(IO(Some(MAuthKeysHelper.getPublicKeyFromString(PUBLIC_KEY))))
    test(client)
    ()
  }
}
