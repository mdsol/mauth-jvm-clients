...
class MAuthRequestSignerSpec extends AnyFlatSpec with Matchers with HttpClient with BeforeAndAfterAll with ScalaFutures {
  ...
  val signerV2 = MAuthRequestSigner(
    UUID.fromString(testUUID),
    FixturesLoader.getPrivateKey,
    epochTimeProvider,
    true
  ...
  val simpleUnsignedRequest = UnsignedRequest(uri = new URI("/"))
  val simpleNewUnsignedRequest = NewUnsignedRequest.fromStringBodyUtf8(httpMethod = "GET", uri = new URI("/"), body = "", headers = Map.empty)

  val EXPECTED_GET_X_MWS_AUTHENTICATION_HEADER_FOR_SIMPLE_REQUEST =
  ...
  val EXPECTED_GET_MCC_AUTHENTICATION_HEADER_FOR_SIMPLE_REQUEST =
