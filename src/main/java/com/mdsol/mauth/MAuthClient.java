...
public class MAuthClient {
   ...
   * @throws Exception
   */
  public MAuthClient(String mAuthUrl, String mAuthRequestUrlPath, String securityTokensUrl,
      String appId, String publicKey, String privateKey) throws Exception {
   ...
   * @throws Exception
   */
  public boolean init(String mAuthUrl, String mAuthRequestUrlPath, String securityTokensUrl,
      String appId, String publicKey, String privateKey) throws Exception {
  ...
  public HttpServletResponse getSignedResponse(HttpServletResponse response) throws Exception {
    ...
    Map<String, String> headers =
        getSignedResponseHeaders(statusCodeString, body, _appId, epochTimeAsString);
  ...
  public Map<String, String> getSignedResponseHeaders(String statusCodeString, String body,
      String appId, String epochTime) throws Exception {
  ...
  /**
   * Validates an incoming http request which contains mAuth headers. The validation process
   * consists on recreating the mAuth hashed signature and comparing it with the decrypted hash
   * signature from the mAuth header. This approach is faster because it computates the hashes
   * locally and the mAuth service is not contacted to validate, but just to get the requester's
   * public key (if not in cache already). There is an additional way to perform an mAuth validation
   * but is is not implemented in this client
   * 
   * @param signatureInHeader the encrypted and hashed mAuth signature in the header
   ...
   * @throws Exception
   */
  public Boolean validateRequest(String signatureInHeader, String epochTime, String verb,
      String resourceUrl, String body, String appId) throws Exception {
    ...
    // We need the public key of the appId from which the request comes from (this appId is part of
    // the mAuth header x-mws-authentication)
    ...
    byte[] decryptedHexMsg_bytes =
        decryptEngine.processBlock(encryptedSignature, 0, encryptedSignature.length);
  ...
  private PublicKey getPublicKey(String appId) throws Exception {
    ...
    String completeMAuthResourceUrlPath =
        _mAuthRequestUrlPath + String.format(_securityTokensUrl, appId); // this appId is the appId
    ...
    try {
      ...
      throw new Exception(
          "Unable to parse jsonResponse from mAuth service while requesting a public key for: "
              + appId,
          ex);
    }
    ...
    try {
      parser = new PEMParser(new StringReader(publicKeyString));
      publicKey = keyConverter.getPublicKey((SubjectPublicKeyInfo) parser.readObject());

      // Add public key to cache
      _publicKeys.put(appId, publicKey);
    } catch (Exception ex) {
   ...
   */
  private String getStringToSign(String verb, String resourceUrl, String body, String appId,
      String epochTime) {
    String stringToSign = verb + "\n" + resourceUrl + "\n" + body + "\n" // TODO: confirm if body
  ...
  /**
   * Gets a private key object from the text version of the private key, and caches it. Typically
   * the private key will belong to a signer appId which in most cases will be the appId of the
   * application calling this client class
   * 
  ...
  private PrivateKey getPrivateKey(String appId) throws Exception {
    ...
    try {
      parser = new PEMParser(new StringReader(_privateKey));
      privateKey =
          keyConverter.getPrivateKey(((PEMKeyPair) parser.readObject()).getPrivateKeyInfo());
    } catch (Exception ex) {
      throw new Exception("Unable to create private key for: " + appId, ex);
    } finally {
      _privateKeys.put(appId, privateKey);
      try {
        if (parser != null) {
          parser.close();
        }
   ...
   * @throws Exception
   */
  public synchronized String callmAuth(String url, Map<String, String> params_header,
      String content, String method) throws Exception {
   ...
   * @throws IOException
   */
  private HttpURLConnection invokeApi(String sURL, Map<String, String> params_header,
      String content, String method) throws MalformedURLException, URISyntaxException, IOException {
    URL url = new URL(sURL);
    /*
     * List<Proxy> proxies = ProxySelector.getDefault().select( new URI(sURL)); Proxy p =
     * (proxies.size() == 0) ? Proxy.NO_PROXY : proxies.get(0);
     */
