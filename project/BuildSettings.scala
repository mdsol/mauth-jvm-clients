...
object BuildSettings {
  ...
  lazy val basicSettings = Seq(
    ...
    Compile / scalacOptions ++= {
      ...
      commonWconf ++ (CrossVersion.partialVersion(scalaVersion.value) match {
    ...
    Test / scalacOptions ++= {
      ...
      commonOpts ++ (CrossVersion.partialVersion(scalaVersion.value) match {
        case Some((2, _)) =>
          List(
            "-Xlint:unused"
          ) ++ (CrossVersion.partialVersion(scalaVersion.value) match {
            case Some((2, n)) if n >= 13 => List("-Wnonunit-statement")
            case _                       => List()
          })
        case Some((3, _)) =>
          List("-Wvalue-discard")
    ...
    scalacOptions --= {
      if (sys.env.contains("CI"))
        ...
        val scala2Only = CrossVersion.partialVersion(scalaVersion.value) match {
          case Some((2, _)) => Seq("-Xlint:unused")
          case _            => Seq.empty
        }
        Seq("-Xfatal-warnings") ++ scala2Only
      }
    }
  )
